...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAndcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAndcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeAndcoinAmount = strAnonymizeAndcoinAmount.remove(strAnonymizeAndcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAndcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAndcoinAmount*COIN) nMaxToAnonymize = nAnonymizeAndcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAndcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAndcoinAmount));
        strAnonymizeAndcoinAmount = strAnonymizeAndcoinAmount.remove(strAnonymizeAndcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAndcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
