...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAndcoinAmount"))
        settings.setValue("nAnonymizeAndcoinAmount", 1000);
    nAnonymizeAndcoinAmount = settings.value("nAnonymizeAndcoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeAndcoinAmount"))
        SoftSetArg("-anonymizeandcoinamount", settings.value("nAnonymizeAndcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeAndcoinAmount:
            return QVariant(nAnonymizeAndcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeAndcoinAmount:
            nAnonymizeAndcoinAmount = value.toInt();
            settings.setValue("nAnonymizeAndcoinAmount", nAnonymizeAndcoinAmount);
            emit AnonymizeAndcoinAmountChanged(nAnonymizeAndcoinAmount);
            break;
        default:
            break;
        }
