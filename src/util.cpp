...
int nAnonymizeAndcoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Andcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Andcoin
    // Mac: ~/Library/Application Support/Andcoin
    // Unix: ~/.andcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AndCoinV2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "andcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "andcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong ANDCOIN will not work properly.");
